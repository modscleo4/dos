...
static void dns_handle_response(ethernet_driver *driver, dns_header *header, size_t data_size) {
    ...
    uint16_t question_count = switch_endian_16(header->questions);
    uint16_t answer_count = switch_endian_16(header->answers);
    uint16_t authority_count = switch_endian_16(header->authority);
    uint16_t additional_count = switch_endian_16(header->additional);
    ...
    for (int i = 0; i < answer_count; i++) {
        char domain[256];
        if (*data & 0xC0) {
            uint16_t offset = (*data & ~0xC0 << 8) | *(data + 1);
        ...
        if (switch_endian_16(answer->type) == DNS_TYPE_A) {
            ...
            dbgprint("Received DNS A response with IP address %d.%d.%d.%d\n", ip[0], ip[1], ip[2], ip[3]);
            dbgprint("Domain: %s\n", domain);
